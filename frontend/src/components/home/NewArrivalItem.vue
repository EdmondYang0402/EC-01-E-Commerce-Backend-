...
<template>
  <RouterLink class="arrival-item" :to="`/products/${product.id}`">
    <div class="arrival-item__image"><SafeImage :src="imageUrl" :fallback="fallbackImage" :alt="product.name" /></div>
...
.arrival-item__image :deep(img) { width: 100%; height: 100%; object-fit: cover; }
