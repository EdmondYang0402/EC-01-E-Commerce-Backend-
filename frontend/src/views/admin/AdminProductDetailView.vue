...
const skuLabels = computed(() => ({ ENABLED: t('admin.common.enabled'), DISABLED: t('admin.common.disabled') }))
const productId = computed(() => Number(route.params.productId))
const editingSku = computed(() => editingSkuId.value != null)
const confirmMessage = computed(() => {
  if (!confirmTarget.value) return ''
  const enabled = ['ON_SHELF', 'ENABLED'].includes(confirmTarget.value.nextStatus)
  const action = enabled ? t('admin.common.enable') : t('admin.common.disable')
  if (confirmTarget.value.type === 'product') return t('admin.productDetail.productConfirm', { action, name: detail.value?.name })
...
const saveSku = async () => {
  if ((!editingSku.value && !skuForm.skuCode.trim()) || !skuForm.specJson.trim() || skuForm.price === '' || skuForm.stock === '') {
    ElMessage.warning(t('admin.productDetail.skuRequired'))
    return
  }
  if (Number(skuForm.price) < 0 || Number(skuForm.stock) < 0) { ElMessage.warning(t('admin.productDetail.nonnegative')); return }
  ...
  try {
    ...
    ElMessage.success(editingSku.value ? t('admin.productDetail.skuUpdated') : t('admin.productDetail.skuAdded'))
    skuEditorOpen.value = false
    await load()
  } catch (requestError) { ElMessage.error(errorMessage(requestError, t('admin.productDetail.skuSaveFailed'))) }
...
const confirmStatus = async () => {
  ...
  try {
    ...
    ElMessage.success(t('admin.productDetail.statusUpdated'))
    confirmTarget.value = null
    await load()
  } catch (requestError) { ElMessage.error(errorMessage(requestError, t('admin.productDetail.statusUpdateFailed'))) }
  ...
  <section class="admin-page">
    ...
    <template v-else>
      ...
      <section class="sku-section">
        <header><div><p class="admin-eyebrow">SKU</p><h2>{{ t('admin.productDetail.skuTitle') }}</h2><span>{{ t('admin.productDetail.skuCount', { count: detail.skus?.length || 0 }) }}</span></div><button class="admin-primary-button" type="button" @click="openCreateSku">{{ t('admin.productDetail.addSku') }}</button></header>
        <div v-if="!detail.skus?.length" class="admin-state sku-empty">{{ t('admin.productDetail.noSku') }}</div>
    ...
    <Teleport to="body">
      <div v-if="skuEditorOpen" class="sku-editor-backdrop" @click.self="closeSkuEditor">
        <section class="sku-editor" role="dialog" aria-modal="true" :aria-label="editingSku ? t('admin.productDetail.editSku') : t('admin.productDetail.createSku')">
          <header><div><p class="admin-eyebrow">SKU</p><h2>{{ editingSku ? t('admin.productDetail.editSku') : t('admin.productDetail.createSku') }}</h2></div><button type="button" :aria-label="t('admin.productDetail.closeEditor')" @click="closeSkuEditor">×</button></header>
          <form @submit.prevent="saveSku">
            <label class="admin-field"><span>{{ t('admin.productDetail.skuCodeRequired') }}</span><input v-model="skuForm.skuCode" maxlength="64" :disabled="editingSku" required placeholder="CHAIR-BLACK" /></label>
            <label class="admin-field"><span>{{ t('admin.productDetail.specJsonRequired') }}</span><textarea v-model="skuForm.specJson" maxlength="500" required :placeholder="t('admin.productDetail.specExample')" /></label>
            <div class="sku-editor__grid"><label class="admin-field"><span>{{ t('admin.productDetail.priceRequired') }}</span><input v-model="skuForm.price" min="0" step="0.01" type="number" required /></label><label class="admin-field"><span>{{ t('admin.productDetail.stockRequired') }}</span><input v-model="skuForm.stock" min="0" step="1" type="number" required /></label></div>
            <p>{{ t('admin.productDetail.statusNote') }}</p>
            <footer><button class="admin-secondary-button" type="button" @click="closeSkuEditor">{{ t('admin.common.cancel') }}</button><button class="admin-primary-button" type="submit" :disabled="skuSaving">{{ skuSaving ? t('admin.common.saving') : t('admin.productDetail.saveSku') }}</button></footer>
          </form>
        </section>
      </div>
    </Teleport>

    <AdminConfirmDialog :open="Boolean(confirmTarget)" :title="t('admin.productDetail.statusConfirm')" :message="confirmMessage" :confirm-text="confirmTarget?.nextStatus === 'OFF_SHELF' || confirmTarget?.nextStatus === 'DISABLED' ? t('admin.productDetail.confirmDeactivate') : t('admin.productDetail.confirmEnable')" :danger="confirmTarget?.nextStatus === 'OFF_SHELF' || confirmTarget?.nextStatus === 'DISABLED'" :busy="statusBusy" @cancel="confirmTarget = null" @confirm="confirmStatus" />
  </section>
</template>
