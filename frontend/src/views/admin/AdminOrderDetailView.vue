...
<template>
  <section class="admin-page">
    ...
    <template v-else>
      <header class="order-hero"><div><p class="admin-eyebrow">{{ t('admin.orders.eyebrow') }}</p><h1>{{ detail.orderNo }}</h1><p>{{ t('admin.orderDetail.created', { userId: detail.userId, date: formatDateTime(detail.createTime) }) }}</p></div><div><AdminStatusBadge :status="detail.status" :labels="statusLabels" /><strong>{{ formatMoney(detail.totalAmount) }}</strong></div></header>
...
.snapshot-list :deep(img) { width: 78px; height: 68px; object-fit: cover; background: var(--paper); }
...
@media (max-width: 720px) { .receiver-panel > div { grid-template-columns: 1fr; } .snapshot-list article { grid-template-columns: 62px 1fr; } .snapshot-list :deep(img) { width: 62px; height: 58px; } .snapshot-list article > span, .snapshot-list article > strong { grid-column: 2; text-align: left; } }
